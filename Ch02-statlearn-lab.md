...
- Access to `Jupyter`, a very popular `Python` interface that runs code through a file called a _notebook_.
...
Like most programming languages, `Python` uses _functions_
to perform operations. To run a
function called `fun`, we type
`fun(input1,input2)`, where the inputs (or _arguments_)
...
_strings_. For instance, `"hello"` and
`'hello'`
are strings.
...
_library_, or _package_. A package is a collection of modules that are not necessarily included in
the base `Python` distribution. The name `numpy` is an abbreviation for _numerical Python_.
...
In `numpy`, an _array_ is a generic term for a multidimensional
set of numbers.
...
_attributes_, or associated objects. To access an attribute of `x`, we type `x.attribute`, where we replace `attribute`
...
Similarly, `x.dtype` is the _data type_ attribute of the object `x`. This indicates that `x` is
...
_floating point numbers_ (i.e. real-valued numbers).
...
A _method_ is a function that is associated with an
object.
...
of _rows_. This is called _row-major ordering_, as opposed to _column-major ordering_.
...
This _signature_ line tells us that the function's arguments are `loc`, `scale`, and `size`. These are _keyword_ arguments, which means that when they are passed into
the function, they can be referred to by name (in any order). {`Python` also uses _positional_ arguments. Positional arguments do not need to use a keyword. To see an example, type in `np.sum?`. We see that `a` is a positional argument, i.e. this function assumes that the first unnamed argument that it receives is the array to be summed. By contrast, `axis` and `dtype` are keyword arguments: the position in which these arguments are entered into `np.sum()` does not matter.}
...
each time it is run, we can set a _random seed_
using the
...
In `matplotlib`, a plot consists of a _figure_ and one or more _axes_. You can think of the figure as the blank canvas upon which
one or more plots will be displayed: it is the entire plotting window.
The _axes_ contain important information about each plot, such as its $x$- and $y$-axis labels,
title, and more. (Note that in `matplotlib`, the word _axes_ is not the plural of _axis_: a plot's _axes_ contains much more information
...
We pause here to note that we have _unpacked_ the tuple of length two returned by `subplots()` into the two distinct
...
`ax.contour()` method produces a _contour plot_
...
_heatmap_, and is sometimes used to plot temperature in
weather forecasts.
...
Why isn't $10$ output above? This has to do with _slice_ notation in `Python`.
Slice notation  
...
The _convenience function_ `np.ix_()` allows us to extract a submatrix
using lists, by creating an intermediate _mesh_ object.
...
Otherwise,
the command
`os.chdir()` can be used to _change directory_. (You will need to call `import os` before calling `os.chdir()`.)
...
value `np.nan`, which means _not a number_:
...
Similarly,
passing in a slice to the `[]` method creates a data frame whose _rows_ are determined by the slice:
...
Since we did not specify an _index_ column when we loaded our data frame, the rows are labeled using integers
0 to 396.
...
We also took advantage of the _increment_ notation
...
We can use the `pd.plotting.scatter_matrix()` function to create a _scatterplot matrix_ to visualize all of the pairwise relationships between the columns in
a data frame.
